...
|----------|-------------|----------------|
| **VBAT+** | Battery positive rail | CN1.1, Q1.VIN(1), Q1.CE(3), C17.+, R7.1 |
| **3V3** | 3.3V regulated rail | Q1.VOUT(5), C16.+, C11.+, U1 VDD pins (11, 27, 50, 51, 75, 100), U1.VDDSMPS(46), U1.VDDA(21), U1.VREF+(20), U1.VDDUSB(73), U1.VBAT(6), C1.+, C2.+, C3.+, C4.+, C5.+, C6.+, C7.+, C8.+, C9.+, C10.+, C14.+, C15.+, R1.1, R3.1, R4.1, COMM1.VCC, CARD1.VDD, CN2.3, H1.1, H2.1, U2.VBKP |
| **GND** | Ground | *(see dedicated GND table below)* |
| **GPS_VCC** | Switched 3.3V to GPS module | Q2.Source, U2.VCC |
| **VDD11** | 1.1V SMPS output | L1.2, U1.pin48, C12.+, C13.+ |
| **VLXSMPS** | SMPS switch node | U1.pin45, L1.1 |
| **VSSSMPS** | SMPS dedicated ground | U1.pin47, C12.−, C13.− |
...
| Group | Pins |
|-------|------|
| MCU VSS | U1.pin19 (VSSA), U1.pin26, U1.pin49, U1.pin74, U1.pin99 |
| LDO | Q1.VSS(2) |
| Decoupling caps (−) | C1.−, C2.−, C3.−, C4.−, C5.−, C6.−, C7.−, C8.−, C9.−, C10.−, C11.−, C14.−, C15.−, C16.−, C17.−, C18.− |
...
> **Note:** VSSSMPS (U1.pin47, C12.−, C13.−) is a separate net — do NOT merge with GND. The SMPS ground connects to the main ground plane on the PCB, but the net port must be distinct in the schematic for proper SMPS routing.
...
| Net Port | Description | Connected Pins |
|----------|-------------|----------------|
| **BLE_TX** | BLE data out (BLE→MCU) | COMM1.TX, U1.pin25 (PA3 / USART2_RX, AF7) |
| **BLE_RX** | BLE data in (MCU→BLE) | COMM1.RX, U1.pin24 (PA2 / USART2_TX, AF7) |
...
| Net Port | Description | Connected Pins |
|----------|-------------|----------------|
| **SD_SCK** | SPI clock | CARD1.CLK, U1.pin29 (PA5 / SPI1_SCK, AF5) |
| **SD_MISO** | SPI data out (SD→MCU) | CARD1.DAT0, U1.pin30 (PA6 / SPI1_MISO, AF5) |
| **SD_MOSI** | SPI data in (MCU→SD) | CARD1.CMD, U1.pin31 (PA7 / SPI1_MOSI, AF5) |
| **SD_CS** | SPI chip select (active low) | CARD1.CD/DAT3, U1.pin28 (PA4, GPIO) |
...
| Net Port | Description | Connected Pins |
|----------|-------------|----------------|
| **PDM_CLK** | PDM clock to mic breakout | CN2.1, U1.pin37 (PE9 / ADF1_CCK0, AF3) |
| **PDM_DATA** | PDM data from mic breakout | CN2.2, U1.pin38 (PE10 / ADF1_SDI0, AF3) |
...
| C18.1 | U1.pin14 (NRST) | Reset filter cap |
| C18.2 | GND | Via GND net port |

---

## MCU Pin Quick-Reference (Assigned Pins Only)

Pin numbers from DS13737 Rev 10, LQFP-100 SMPS variant.
...
| 17 | PC2 | GPS_PPS | EXTI |
| 24 | PA2 | BLE_RX | USART2_TX (AF7) |
| 25 | PA3 | BLE_TX | USART2_RX (AF7) |
| 28 | PA4 | SD_CS | GPIO output |
| 29 | PA5 | SD_SCK | SPI1_SCK (AF5) |
| 30 | PA6 | SD_MISO | SPI1_MISO (AF5) |
...
| C2 | 100nF | VDD pin 27 | 3V3 | GND |
| C3 | 100nF | VDD pin 50 | 3V3 | GND |
| C4 | 100nF | VDD pin 51 | 3V3 | GND |
| C5 | 100nF | VDD pin 75 | 3V3 | GND |
| C6 | 100nF | VDD pin 100 | 3V3 | GND |
| C7 | 100nF | VDDA pin 21 | 3V3 | GND |
| C8 | 100nF | VREF+ pin 20 | 3V3 | GND |
| C9 | 100nF | VDDUSB pin 73 | 3V3 | GND |
| C10 | 100nF | VBAT pin 6 | 3V3 | GND |
| C11 | 10uF | VDD bulk (near pin 11) | 3V3 | GND |
| C12 | 2.2uF | VDD11 SMPS out 1 | VDD11 | VSSSMPS |
| C13 | 2.2uF | VDD11 SMPS out 2 | VDD11 | VSSSMPS |
| C14 | 1uF | VDDA pin 21 | 3V3 | GND |
| C15 | 1uF | VREF+ pin 20 | 3V3 | GND |
| C16 | 1uF | LDO output (Q1.VOUT) | 3V3 | GND |
| C17 | 1uF | LDO input (Q1.VIN) | VBAT+ | GND |
| C18 | 100nF | NRST pin 14 | *(local)* | GND |

**Totals:** 11x 100nF, 1x 10uF, 2x 2.2uF, 4x 1uF = 18 caps.
...
- [ ] All 18 decoupling caps connected (11x 100nF + 1x 10uF + 2x 2.2uF + 4x 1uF)
- [ ] SMPS: L1 between pins 45↔48, C12/C13 between pin 48↔47
